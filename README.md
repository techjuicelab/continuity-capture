...
```sh
open -na ~/Documents/Dev/AI/MacStudioM2Max/continuity-camera/ContinuityCapture.app --args photo
open -na ~/Documents/Dev/AI/MacStudioM2Max/continuity-camera/ContinuityCapture.app --args scan
```
...
```sh
./build.sh   # swiftc (Command Line Tools) 필요, ad-hoc 서명
```
...
git clone git@github.com:techjuicelab/continuity-capture.git \
    ~/Documents/Dev/AI/MacStudioM2Max/continuity-camera
cd ~/Documents/Dev/AI/MacStudioM2Max/continuity-camera && ./build.sh
```

이후 `shortcuts/`의 두 파일을 더블클릭해 가져오고 키보드 단축키를 지정하면 끝.
단축어는 앱 경로를 `$HOME` 기준으로 참조하므로 위 경로에 클론하면 사용자명이
달라도 동작한다. `.app` 번들은 빌드 산출물이라 저장소에 포함하지 않는다.
